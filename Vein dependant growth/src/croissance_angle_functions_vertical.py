...
D_VEIN = 100/6 * 4/3   #critical distance for vein_propagation()
C = 2
S_GROWTH = D_VEIN / C
...
DT = 0.05
T = 100000
CENTRAL_WEIGHT = 1  #tune how the growth is more important for the central vein (it's a factor)
BASAL_WEIGHT = 1
...
@jit
def growth_vein_obweight(growth_front, list_veins, a0 = A0, am = AM, s_growth = S_GROWTH , d_vein = D_VEIN, dt = DT, threshold = 10):
...
def f_theta_exp(x, th_0 = 80, th_1 = 160, measure = 'radians'):
...
@jit
def growth_vein_angle(growth_front, list_veins, iteration, a0 = A0, am = AM, s_growth = S_GROWTH , d_vein = D_VEIN, dt = DT, threshold = 10):
...
def propagate_veins_weightvertical(growth_front, list_veins, list_junctions, iteration_time, d_vein = D_VEIN, central_weight = CENTRAL_WEIGHT, basal_weight = BASAL_WEIGHT):
    ...
    if len(veins) <= 1:
        ...
        if growth_front[6,v1] + growth_front[6,v2] >= 2 * d_vein:
            ...
            if mother_weight == basal_weight:
                ...
                #  find the most vertical vein
                
                # compute theta for third_1
                u1, v1 = growth_front[3,third_1], growth_front[4,third_1]
                if v1 >= 0:
                    theta_1 = math.atan(u1 / v1)
            
                else:
                    if u1 >= 0:
                        theta_1 = math.atan(u1 / v1) + math.pi 
                    else:
                        theta_1 = math.atan(u1 / v1) - math.pi 
                    
                # compute theta for third_2
                u2, v2 = growth_front[3,third_2], growth_front[4,third_2]
                if v2 >= 0:
                    theta_2 = math.atan(u2 / v2)
            
                else:
                    if u2 >= 0:
                        theta_2 = math.atan(u2 / v2) + math.pi 
                    else:
                        theta_2 = math.atan(u2 / v2) - math.pi                
